...
CASES = [
    ...
    ("period change", L.compute_period_change,
     PrimitiveArgs(flow="gpr", metric="premium", filters={"Carrier_Group": "Zurich"}),
     {"grain": "quarter"}),
    ("aligned periods", L.compute_aligned_periods,
     PrimitiveArgs(flow="gpr", metric="premium", filters={"Carrier_Group": "Zurich"}),
     {"grain": "quarter"}),
    ("aligned periods by product", L.compute_aligned_periods,
     PrimitiveArgs(flow="gpr", metric="premium", group_by=("Product_Line",),
                   filters={"Carrier_Group": "Zurich"}), {"grain": "quarter"}),
    # A year pinned in the filters must be DROPPED by both executors, or there is
    # only one year left and nothing to compare it against.
    ("aligned periods pinned to one year", L.compute_aligned_periods,
     PrimitiveArgs(flow="gpr", metric="premium", filters=ZURICH), {"grain": "quarter"}),
    ("contribution", L.compute_contribution,
     PrimitiveArgs(flow="gpr", metric="premium", group_by=("Product_Line",),
                   filters={"Carrier_Group": "Zurich"}), {}),
    ("contribution by industry", L.compute_contribution,
     PrimitiveArgs(flow="gpr", metric="premium", group_by=("SIC_Major_Class",),
                   filters={"Carrier_Group": "Zurich"}), {}),
    ("contribution with no cut", L.compute_contribution,
     PrimitiveArgs(flow="gpr", metric="premium", filters={"Carrier_Group": "Zurich"}), {}),
