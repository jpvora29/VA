...
_JUDGE_SYSTEM = (
    ...
    "A bullet MAY open with a short lead-in label naming the industry, segment or product it is about, "
    "then a colon, then the finding ('Manufacturing growth: ...'). KEEP that shape. DROP it when "
    "the label repeats the column heading rather than naming a thing, when what follows the colon is "
    "a caption rather than a sentence that could stand without the label, or when the sentence WANDERS "
    "OFF THE LABEL — a bullet headed 'Manufacturing' that also reports Marine or the carrier overall "
    "has mislabelled itself, and those findings belong in bullets of their own.\n"
