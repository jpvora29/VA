...
The portfolio pages carry three table shapes the generic slot path cannot fill:

  * the per-country **feedback table** ("What's working well · What's not · Growth
    Opportunities" with a Marsh GWP / Carrier GWP / Carrier Rank / Carrier SoW callout
    block per country row);
  * the **quadrant table** on the per-country "Carrier:" slide (Successes · Challenges ·
    Opportunities · Key Messages);
  * the one-cell **Key Highlights** table on the ranking slide.

Detection is generic (header/caption text, not slide indices) so it survives template
edits, mirroring :mod:`studio.template_fill.grids`: ``augment`` re-binds each cell to a
positional ``fb:<slide>:<shape>:<r>:<c>`` role; ``values`` computes the texts from the
deterministic compute layer — every sentence is assembled around real figures, and the
optional LLM only re-words it behind the faithfulness verifier.
"""
...
# Cell-caption text → the KPI that cell displays (feedback-table callout block).
...
def _targets(template: Template) -> List[Dict[str, Any]]:
    """Every fillable table cell: ``{slide_idx, shape_id, r, c, kind, country_ord}``."""
    ...
    for slide in template.slides:
        for sh in slide.shapes:
            ...
            for detect in _DETECTORS:
                cells = detect(sh)
                if cells:
                    out.extend({"slide_idx": slide.index, "shape_id": sh.shape_id,
                                "r": r, "c": c, "kind": kind, "country_ord": ordn}
...
def augment(template: Template, bindings: List[R.Binding]) -> List[R.Binding]:
    """Re-bind (or add) each detected table cell to its ``fb:`` role (idempotent)."""
    ...
    for t in _targets(template):
        ...
        if existing is not None:
            ...
            sh = template.shape(t["slide_idx"], t["shape_id"])
            token = ""
            if sh and sh.table and t["r"] < len(sh.table) and t["c"] < len(sh.table[t["r"]]):
                token = sh.table[t["r"]][t["c"]]
            extra.append(R.Binding(
                slot=Slot(t["slide_idx"], t["shape_id"], ["cell", t["r"], t["c"]],
                          token, "text", ""),
                role=role, placeholder=False))
        n += 1
    if n:
        logger.info("feedback: bound %d table cell(s) (%d added)", n, len(extra))
...
def _facts(result, filters: Dict[str, Any]) -> Dict[str, Any]:
    """Carrier/Marsh totals + YoY, rank and SoW movement for one filter scope."""
    ...
    return {"carrier": carrier, "marsh": marsh, "rank": rank, "sow": sow}
...
def _kpi_cell(kind: str, f: Dict[str, Any]) -> str:
    ...
    if kind == "marsh":
        ...
        if t.get("current") is None:
            return ""
        d = t.get("pct")
        head = _money(t["current"]) + (f" ({d:+.1f}%{_arrow(d)})" if d is not None else "")
        return f"{head}\nMarsh GWP"
    if kind == "carrier":
        t = f["carrier"]
        if t.get("current") is None:
    ...
    if kind == "sow":
        ...
        if t.get("current") is None:
            return ""
        d = t.get("delta")
        head = f"{t['current']:.1f}%" + (f" ({d:+.1f}%{_arrow(d)})" if d is not None else "")
        return f"{head}\nCarrier SoW"
    return ""
...
def _polish(text: str, kind: str, style: Optional[str]) -> str:
    ...
    if kind in ("marsh", "carrier", "rank", "sow", "highlights") or not text:
        return text
